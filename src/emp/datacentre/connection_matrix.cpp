...
void ConnectionMatrix::setTopoFlowsC2C(string conn_matrix_str, double simtime_ms) {
  cout << "Topo flows C2C (rack-based)" << endl;
  ...
  cout << "c=" << c << ",seednum=" << seednum << ",rackbase=" << rackbase << ",is_bidirectional=" << isbidirectional << endl;

  // Read traffic from config file
  string trafficfilename = "trafficfiles/c2c/"+rackbase+"_"+to_string(c)+"_"+to_string(isbidirectional)+"_"+to_string(seednum)+".txt";
  cout << "trafficfilename: " << trafficfilename << endl;
  ifstream TMFile(trafficfilename.c_str());
  string line;
  line.clear();
...
void ConnectionMatrix::setTopoFlowsEval(string cmfile) {
