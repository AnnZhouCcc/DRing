...
def simple_quantize(indexed_values):
    quantized_values = [(i, int(round(v*100000000))) for i, v in indexed_values]
    
    current_sum = sum(v for _, v in quantized_values)

    difference = current_sum - 100000000
    # print(difference)

    # adjust on the entry with the largest value
    if difference:
        # Sort indices by quantized values to adjust the largest values first
        sorted_values = sorted(quantized_values, key=lambda x: (x[1], random.random()), reverse=True)
        i, value = sorted_values[0]
        new_value = value - difference
        # print(f"Adjusting index {i} from {value} to {new_value}")
        quantized_values = [(idx, new_value if idx == i else v) for idx, v in quantized_values]

    # current_sum = sum(v for _, v in quantized_values)
    # difference = current_sum - 1000000
    # if difference:
    #     print("ERROR: quantization still has difference")
    #     print(difference)

    return [(i, v/100000000) for i, v in quantized_values]
...
for fromsw in range(numsw):
    for tosw in range(numsw):
        if len(weightarr[fromsw][tosw])>0:
            qweightarr[fromsw][tosw] = simple_quantize(weightarr[fromsw][tosw])
