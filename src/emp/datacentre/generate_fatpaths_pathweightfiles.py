# The difference is that we restrict the number of paths used between each pair of switches to be at most 3
# i.e. Added constraint 2 in the MCF formulation
...
for fromsw in range(numsw):
    bvararr.append(list())
    for tosw in range(numsw):
        bvararr[fromsw].append(list())
        for pid in range(maxpid):
            bvararr[fromsw][tosw].append(None)
for fromsw in range(numsw):
    for tosw in range(numsw):
        if traffic[fromsw][tosw] > 0:
            for pid in range(len(netpath[fromsw][tosw])):
                # Use BINARY variable to indicate whether this path is used
                var = model.addVar(vtype=GRB.BINARY, name=f"b_{fromsw}_{tosw}_{pid}")
                bvararr[fromsw][tosw][pid] = var
...
# Constraint 2: at most 3 paths per switch pair
for fromsw in range(numsw):
    for tosw in range(numsw):
        if len(netpath[fromsw][tosw]) > 0:
            varlist = list()
            for pid in range(len(netpath[fromsw][tosw])):
                if bvararr[fromsw][tosw][pid] is not None:
                    varlist.append(bvararr[fromsw][tosw][pid])
            model.addConstr(sum(varlist)<=3,f"c2_{fromsw}_{tosw}")

M = 1e9  # some big upper bound on flow
for fromsw in range(numsw):
    for tosw in range(numsw):
        for pid in range(len(netpath[fromsw][tosw])):
            if bvararr[fromsw][tosw][pid] is not None:
                # enforce: if vararr > 0 → bvararr must be 1
                model.addConstr(vararr[fromsw][tosw][pid] <= M * bvararr[fromsw][tosw][pid],
                                f"c3_{fromsw}_{tosw}_{pid}")
