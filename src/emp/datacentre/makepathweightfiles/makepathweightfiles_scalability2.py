...
multstrlist = ["0_47_100","0_41_100","0_36_100","0_32_100","0_29_100"]
# multstrlist = ["0_32_100"]
...
for inumsw,numsw in enumerate(swlist):
    ...
    flowfile = f"{homedir}/DRing/src/emp/datacentre/flowfiles/c2s_{numserver}_{numsw}_{numport}_{c}_{s}_{multstr}_{tseed}"

    if graphname=="dring" or graphname=="rrg":
        netpathfile = f"{homedir}/DRing/src/emp/datacentre/netpathfiles/netpath_{routingname}_{graphname}_{numserver}_{numsw}_{numport}"
    else:
        print("ERROR: leafspine does not need to compute path weight")

    if graphname=="dring":
        graphfile = f"{homedir}/DRing/src/emp/datacentre/scalegraphfiles2/{graphname}_deg{numport}_sw{numsw}_sn{numsw//swpersn}_i1.edgelist"
    elif graphname=="rrg":
        graphfile = f"{homedir}/DRing/src/emp/datacentre/scalegraphfiles2/{graphname}_deg{numport}_sw{numsw}_svr{numserver}_os1_i1.edgelist"
    else:
        print("ERROR: leafspine does not need to compute path weight")
