...
routing=$2
trafficmatrix=$3 # kcluster_2/12/24 or ml0
mode=$4 # kcluster # ml
trafficfilename=$5 # a/b/c or 0/1/2/3/4
stime=$6
tag=$7
testpoint=$8
...
then
  ...
  then
    ...
    then
      pwfile=pathweightfiles/${topology}/${routing}/pathweight_${topology}_${routing}_equal_${precision}.txt
    elif [ $topology = "rrg" ] || [ $topology = "dring" ]
    then
      pwfile=pathweightfiles/${topology}/${routing}/cluster${trafficfilename}/pathweight_pbr1_${topology}_${routing}_cluster${trafficfilename}_lp1_${lpsolvermode}_
    fi
  elif [ $mode = "ml" ]
  then
    if [ $topology = "leafspine" ]
...
elif [ $trafficmatrix = "kcluster_2" ]
then
  trafficmatrixparam=KCLUSTER_2
elif [ $trafficmatrix = "kcluster_12" ]
then
  trafficmatrixparam=KCLUSTER_12
...
# Set up a logfile.
dir=discover_${topology}_${routing}_${trafficmatrix}_${mode}_${tag}
if [ ! -d $dir ]
then
  mkdir $dir
fi
...
echo testpoint=$testpoint >> $logfile
...
suffix=${topology}_${routing}_${trafficmatrix}_${mode}_${tag}
tempoutputfile=${dir}/${suffix}_output
make=MAKE

./discover_aux_fraction.sh $testpoint > $tempoutputfile
...
### Search start.
echo $(date): Run experiment test point >> $logfile
...
#       echo ========='['${searchstart},${searchmid}']'========= >> $logfile
#       ./tagrigorous.sh $searchstart $searchmid $topology $routing $trafficmatrix $mode $threshold $stime $mstart $mend $precision $seedfrom $seedto 0 9 $npfile $pwfile $graphname $graphfile $numservers $trafficmatrixparam $routingparam $kornparam $solvestart $solveend $trafficfilename $dp $solveinterval $computestart $computeend $computeinterval $tag > $tempoutputfile
...
#       echo ========='['${searchmid},${searchend}']'========= >> $logfile
#       ./tagrigorous.sh $searchmid $searchend $topology $routing $trafficmatrix $mode $threshold $stime $mstart $mend $precision $seedfrom $seedto 0 9 $npfile $pwfile $graphname $graphfile $numservers $trafficmatrixparam $routingparam $kornparam $solvestart $solveend $trafficfilename $dp $solveinterval $computestart $computeend $computeinterval $tag > $tempoutputfile
#       x=$(cat $tempoutputfile | cut -d " " -f 1)
#       echo $x
#     else
