...
# Set up a logfile.
dir=oblivious_routing_discover_testopt_${topology}_${routing}_${trafficmatrix}_${mode}_${norc}_${seednumpw}_${seednumtraffic}_$pwnum
if [ ! -d $dir ]
then
  mkdir $dir
fi
...
suffix=${topology}_${routing}_${trafficmatrix}_${mode}_${norc}_${seednumpw}_${seednumtraffic}_$pwnum
...
do
  ...
  then
    ...
    then
      ...
      echo ========='['${searchstart},${searchmid}']'========= >> $logfile
      ./oblivious_routing_rigorous_testopt.sh $searchstart $searchmid $topology $routing $trafficmatrix $mode $threshold $stime $mstart $mend $precision $seedfrom $seedto 0 9 $npfile $pwfile $graphname $graphfile $numservers $trafficmatrixparam $routingparam $kornparam $solvestart $solveend $trafficfilename $dp $solveinterval $computestart $computeend $computeinterval $decaymode $norc $seednumpw $seednumtraffic $pwnum > $tempoutputfile
    ...
    then
      ...
      echo ========='['${searchmid},${searchend}']'========= >> $logfile
      ./oblivious_routing_rigorous_testopt.sh $searchmid $searchend $topology $routing $trafficmatrix $mode $threshold $stime $mstart $mend $precision $seedfrom $seedto 0 9 $npfile $pwfile $graphname $graphfile $numservers $trafficmatrixparam $routingparam $kornparam $solvestart $solveend $trafficfilename $dp $solveinterval $computestart $computeend $computeinterval $decaymode $norc $seednumpw $seednumtraffic $pwnum > $tempoutputfile
      x=$(cat $tempoutputfile | cut -d " " -f 1)
      echo $x
    else
...
if [ $topology = "leafspine" ]
then
  rm leafspine_${topology}_${routing}_${trafficmatrix}_${mode}_${norc}_${seednumpw}_${seednumtraffic}_$pwnum
else
  rm rrg_${topology}_${routing}_${trafficmatrix}_${mode}_${norc}_${seednumpw}_${seednumtraffic}_$pwnum
fi
