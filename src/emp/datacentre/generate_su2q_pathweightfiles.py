...
# 🔄 CHANGED: Added `pathlength` as a parameter
def quantize_and_adjust_with_indices(indexed_values, factor, pathlength, isprint=False):
    ...
    if difference != 0:
        if difference > 0:
            # 🔄 CHANGED: Combine with pathlength and random tie-breaker
            combined = [(i, v, pathlength[i], random.random()) for i, v in quantized_values]
            # Sort by value descending, then pathlength ascending, then random
            sorted_quantized = sorted(combined, key=lambda x: (-x[1], x[2], x[3]))
            if isprint: print(f"Sorted for decrease: {sorted_quantized}")

            while difference > 0:
                for i, value, _, _ in sorted_quantized:
                    if difference <= 0:
                        break
            ...
            # 🔄 CHANGED: Combine with pathlength and random tie-breaker
            combined = [(i, v, pathlength[i], random.random()) for i, v in quantized_values]
            # Sort by value ascending, then pathlength ascending, then random
            sorted_quantized = sorted(combined, key=lambda x: (x[1], x[2], x[3]))
            if isprint: print(f"Sorted for increase: {sorted_quantized}")
...
for fromsw in range(numsw):
    for tosw in range(numsw):
        if len(weightarr[fromsw][tosw])>0:
            pathlength = dict()
            for pid,_ in weightarr[fromsw][tosw]:
                pathlength[pid] = len(netpath[fromsw][tosw][pid])
            qweightarr[fromsw][tosw] = quantize_and_adjust_with_indices(weightarr[fromsw][tosw],factor,pathlength)
