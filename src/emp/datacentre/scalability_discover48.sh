...
elif [ $trafficmatrix = "16to4-5" ]
then
  trafficmatrixparam=S2S_16_4_0_5
elif [ $trafficmatrix = "flat-16to4-0" ]
then
  trafficmatrixparam=F2F_16_4_0_0
elif [ $trafficmatrix = "48to48-0" ]
then
  trafficmatrixparam=S2S_48_48_0_0
...
do
  ...
  do
    ...
    if [ ! -s $outputfile ]
    then
      time ./run.sh $graphname 1 60 16 $graphfile $numservers 1 1 $make $trafficmatrixparam $mult $numerator $denominator $solvestart $solveend $routingparam $kornparam $trafficfilename 0 $seed $suffix $npfile $pwfile $dp $mstart $mend $stime $solveinterval $computestart $computeend $computeinterval | grep -e "FCT" -e "topology" > $outputfile &
      sleep 5
      wait
      echo $(date): Experiment done "("seed=$seed")" >> $logfile
      make=NOMAKE
