...
if [[ $1 == "LEAFSPINE" ]]; then
    ...
    echo $tempResultFile
    ./leafspine_${SUFFIX} -o ${tempLogFile} -sub $2 -TMatrix ${10} -mult ${MULT} -numerator ${NUMERATOR} -denominator ${DENOMINATOR} -r ${ROUTING} -k ${KorN} -param ${PARAM} -paramo ${PARAMO} -netpath ${NETPATH} -pathweight ${PATHWEIGHT} | tee $tempResultFile
...
if [[ $1 == "RRG" ]]; then
    ...
    echo ${PARAM}
    echo ${PARAMO}
    ./rrg_$SUFFIX -o ${tempLogFile} -sub $2 -TMatrix ${TMatrix} -mult ${MULT} -numerator ${NUMERATOR} -denominator ${DENOMINATOR} -r ${ROUTING} -k ${KorN} -param ${PARAM} -paramo ${PARAMO} -topo $5 -seed ${SEED} -netpath ${NETPATH} -pathweight ${PATHWEIGHT} | tee $tempResultFile
