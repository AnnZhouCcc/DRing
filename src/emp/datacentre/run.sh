...
if [[ $1 == "RRG" ]]; then
    ...
    echo ${PARAM}
    echo ${PARAMO}
    ./rrg_$SUFFIX -o ${tempLogFile} -sub $2 -TMatrix ${TMatrix} -mult ${MULT} -numerator ${NUMERATOR} -denominator ${DENOMINATOR} -r ${ROUTING} -k ${KorN} -param ${PARAM} -paramo ${PARAMO} -topo $5 -seed ${SEED} -netpath ${NETPATH} -pathweight ${PATHWEIGHT} -dp ${DP} | tee $tempResultFile
