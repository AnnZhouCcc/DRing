...
routing=$2
trafficmatrix=$3 #c2s, clustera, clusterb, clusterc
trafficmatrixbase=leafspine
norc=$5
nors=$6
seednumpw=0 # used only in oblivious-ecmp-opt
seednumtraffic=$7
pwnum=$8
mode=$9 # oblivious, oblivious-ecmp-equal
...
if [ ! $lpsolvermode = "barriernocrossover" ] && [ ! $lpsolvermode = "null" ] && [ ! $lpsolvermode = "barrierwithcrossover" ]
then
...
then
  ...
  elif [ $mode = "kawa1" ] || [ $mode = "kawa2" ]
  then
    pwfile=/home/annzhou/PathWeightFileGenerator/pathweightfiles/sud_robustness/pathweight_${topology}_${routing}_${mode}_${trafficmatrix}_${lpsolvermode}_${precision}.txt
  elif [ $mode = "oblivious" ]
  then
  ...
  then
    ...
    then
      ...
      pwfile=/home/annzhou/PathWeightFileGenerator/pathweightfiles/c2c_${norc}_0_${seednumpw}_${trafficmatrixbase}/pathweight_${topology}_${routing}_c2c_${norc}_0_${seednumpw}_${trafficmatrixbase}_lp1_${lpsolvermode}_${precision}.txt
    fi
...
elif [ $trafficmatrix = "1920to1920_standard" ]
then
  trafficmatrixparam=SVR_1920_1920_standard_0
elif [ $trafficmatrix = "2304to2304_standard" ]
then
  trafficmatrixparam=SVR_2304_2304_standard_0
elif [ $trafficmatrix = "2688to2688_standard" ]
then
  trafficmatrixparam=SVR_2688_2688_standard_0
...
then
  trafficmatrixparam=SVR_${norc}_${norc}_standard_$seednumtraffic
elif [ $trafficmatrix = "c2c" ]
then
  trafficmatrixparam=C2C_${trafficmatrixbase}_${norc}_0_$seednumtraffic
elif [ $trafficmatrix = "c2s" ]
then
  trafficmatrixparam=C2S_${trafficmatrixbase}_${norc}_${nors}_0_$seednumtraffic
else
...
# Set up a logfile.
dir=oblivious_c2s_discover_${topology}_${routing}_${trafficmatrix}_${mode}_${norc}_${nors}_${seednumpw}_${seednumtraffic}_$pwnum
if [ ! -d $dir ]
then
  mkdir $dir
fi
...
suffix=${topology}_${routing}_${trafficmatrix}_${mode}_${norc}_${nors}_${seednumpw}_${seednumtraffic}_$pwnum
...
do
  ...
  then
    ...
    then
      ...
      echo ========='['${searchstart},${searchmid}']'========= >> $logfile
      ./oblivious_c2s_rigorous.sh $searchstart $searchmid $topology $routing $trafficmatrix $mode $threshold $stime $mstart $mend $precision $seedfrom $seedto 0 9 $npfile $pwfile $graphname $graphfile $numservers $trafficmatrixparam $routingparam $kornparam $solvestart $solveend $trafficfilename $dp $solveinterval $computestart $computeend $computeinterval $decaymode $norc $nors $seednumpw $seednumtraffic $pwnum > $tempoutputfile
    ...
    then
      ...
      echo ========='['${searchmid},${searchend}']'========= >> $logfile
      ./oblivious_c2s_rigorous.sh $searchmid $searchend $topology $routing $trafficmatrix $mode $threshold $stime $mstart $mend $precision $seedfrom $seedto 0 9 $npfile $pwfile $graphname $graphfile $numservers $trafficmatrixparam $routingparam $kornparam $solvestart $solveend $trafficfilename $dp $solveinterval $computestart $computeend $computeinterval $decaymode $norc $nors $seednumpw $seednumtraffic $pwnum > $tempoutputfile
      x=$(cat $tempoutputfile | cut -d " " -f 1)
      echo $x
    else
...
if [ $topology = "leafspine" ]
then
  rm leafspine_${topology}_${routing}_${trafficmatrix}_${mode}_${norc}_${nors}_${seednumpw}_${seednumtraffic}_$pwnum
else
  rm rrg_${topology}_${routing}_${trafficmatrix}_${mode}_${norc}_${nors}_${seednumpw}_${seednumtraffic}_$pwnum
fi
